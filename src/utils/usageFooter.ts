...
export function formatUsageFooter(
  ...
  const parts = [`[usage] request_tokens=${requestTokens}`, `day_used=${usage.usedTokens}`, `day_limit=${usage.maxTokens}`];
  if (typeof opts.requestEstimatedCostUsd === "number" && Number.isFinite(opts.requestEstimatedCostUsd)) {
    parts.push(`request_est_cost_usd=${formatUsd(opts.requestEstimatedCostUsd)}`);
  }
