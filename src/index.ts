...
const PROJECT_NAME = "Gemini MCP Bridge";
...
function parseArgs(argv: string[]): CliCommand {
  ...
  while (args.length > 0) {
    ...
    if (a === "--http") {
      if (transportOverride && transportOverride !== "http") return { kind: "help" };
    ...
    if (a === "--http-port") {
      ...
      if (!Number.isFinite(parsed) || parsed <= 0 || parsed > 65535) return { kind: "help" };
      httpPort = parsed;
      continue;
    }
    if (a === "--doctor") {
    ...
    if (a === "--stdio") {
      if (transportOverride && transportOverride !== "stdio") return { kind: "help" };
...
function printHelp(info: { name: string; version: string }): void {
  ...
  process.stdout.write(`  gemini-mcp-bridge --http [--http-host host] [--http-port port] [--config path]\n`);
  process.stdout.write(`  gemini-mcp-bridge --print-config [--config path]\n`);
  process.stdout.write(`  gemini-mcp-bridge --doctor [--config path] [--check-api]\n`);
  process.stdout.write(`\n`);
}

async function runDoctor(configPath?: string, checkApi = false): Promise<number> {
  ...
  checks.push({
    name: "oauth_scopes",
    ok: true,
    message: config.auth.oauthScopes.length > 0 ? config.auth.oauthScopes.join(", ") : "none",
  });
  ...
  checks.push({
    name: "api_key_env",
    ok: true,
    message: apiKeyEnv ? `set (${config.auth.apiKeyEnvVar})` : `missing (${config.auth.apiKeyEnvVar})`,
  });
  checks.push({
    name: "api_key_env_alt",
    ok: true,
    message: apiKeyAlt ? `set (${config.auth.apiKeyEnvVarAlt})` : `missing (${config.auth.apiKeyEnvVarAlt})`,
  });
  checks.push({
    name: "api_key_file_env",
    ok: true,
    message: apiKeyFileEnv ? `set (${config.auth.apiKeyFileEnvVar})` : `missing (${config.auth.apiKeyFileEnvVar})`,
  });
  checks.push({
    name: "oauth_token_env",
    ok: true,
    message: oauthTokenEnv ? "set (GEMINI_MCP_OAUTH_TOKEN)" : "missing (GEMINI_MCP_OAUTH_TOKEN)",
  });
  checks.push({
    name: "google_application_credentials",
    ok: true,
    message: googleCredsEnv ? "set (GOOGLE_APPLICATION_CREDENTIALS)" : "missing (GOOGLE_APPLICATION_CREDENTIALS)",
  });
  ...
  try {
    ...
    if (checkApi) {
      const clientConfig =
        resolved.type === "oauth"
          ? { accessToken: resolved.accessToken, baseUrl: config.apiBaseUrl, timeoutMs: 15000 }
          : { apiKey: resolved.apiKey, baseUrl: config.apiBaseUrl, timeoutMs: 15000 };
  ...
  process.stdout.write(JSON.stringify({ ok, timestamp, checks, nextSteps }, null, 2) + "\n");
  return ok ? 0 : 1;
}
...
async function main(): Promise<void> {
  ...
  if (transportMode === "http") {
    const httpHandle = await startHttpServer(sharedDeps, pkg, { host: httpHost, port: httpPort });
    ...
    logger.info("Server running on stdio", { name: pkg.name, version: pkg.version });
