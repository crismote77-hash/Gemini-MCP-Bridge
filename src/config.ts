  ...
  .object({
    auth: z
      .object({
        ...
        oauthScopes: z.array(z.string()).default(["https://www.googleapis.com/auth/generative-language"]),
      })
      .default({}),
    ...
    images: z
      .object({
        ...
        allowedMimeTypes: z.array(z.string()).default(["image/png", "image/jpeg", "image/webp"]),
      })
      .default({}),
    conversation: z
      .object({
...
export function loadConfig(opts: { configPath?: string; env?: NodeJS.ProcessEnv } = {}): BridgeConfig {
  const env = opts.env ?? process.env;

  const resolvedDefaultPath = expandHome(DEFAULT_CONFIG_PATH);
  const resolvedProvidedPath = opts.configPath ? expandHome(opts.configPath) : undefined;
  const configPathToUse = resolvedProvidedPath ?? (fs.existsSync(resolvedDefaultPath) ? resolvedDefaultPath : undefined);

  const fileConfigRaw = configPathToUse ? readJsonFileIfExists(configPathToUse) : undefined;
  const fileConfigObj = isPlainObject(fileConfigRaw) ? fileConfigRaw : {};

  const merged: Record<string, unknown> = mergeDeep(configSchema.parse({}) as Record<string, unknown>, fileConfigObj);

  if (env.GEMINI_MCP_API_KEY) merged.auth = { ...(merged.auth as object), apiKey: env.GEMINI_MCP_API_KEY };
  if (env.GEMINI_MCP_AUTH_MODE) merged.auth = { ...(merged.auth as object), mode: env.GEMINI_MCP_AUTH_MODE };
  if (env.GEMINI_MCP_API_KEY_ENV_VAR)
    merged.auth = { ...(merged.auth as object), apiKeyEnvVar: env.GEMINI_MCP_API_KEY_ENV_VAR };
  if (env.GEMINI_MCP_API_KEY_ENV_VAR_ALT)
    merged.auth = { ...(merged.auth as object), apiKeyEnvVarAlt: env.GEMINI_MCP_API_KEY_ENV_VAR_ALT };
  if (env.GEMINI_MCP_API_KEY_FILE)
    merged.auth = { ...(merged.auth as object), apiKeyFileEnvVar: env.GEMINI_MCP_API_KEY_FILE };
  if (env.GEMINI_MCP_OAUTH_SCOPES)
    merged.auth = { ...(merged.auth as object), oauthScopes: parseListEnv(env.GEMINI_MCP_OAUTH_SCOPES) };
  if (env.GEMINI_MCP_API_BASE_URL) merged.apiBaseUrl = env.GEMINI_MCP_API_BASE_URL;
  if (env.GEMINI_MCP_MODEL) merged.model = env.GEMINI_MCP_MODEL;
  if (env.GEMINI_MCP_TIMEOUT_MS) merged.timeoutMs = parseIntEnv(env.GEMINI_MCP_TIMEOUT_MS, "GEMINI_MCP_TIMEOUT_MS");
  ...
  if (env.GEMINI_MCP_MAX_OUTPUT_TOKENS)
    merged.generation = {
      ...(merged.generation as object),
      maxOutputTokens: parseIntEnv(env.GEMINI_MCP_MAX_OUTPUT_TOKENS, "GEMINI_MCP_MAX_OUTPUT_TOKENS"),
    };

  if (env.GEMINI_MCP_MAX_TOKENS)
    merged.limits = {
      ...(merged.limits as object),
      maxTokensPerRequest: parseIntEnv(env.GEMINI_MCP_MAX_TOKENS, "GEMINI_MCP_MAX_TOKENS"),
    };
  if (env.GEMINI_MCP_MAX_INPUT_CHARS)
    merged.limits = {
      ...(merged.limits as object),
      maxInputChars: parseIntEnv(env.GEMINI_MCP_MAX_INPUT_CHARS, "GEMINI_MCP_MAX_INPUT_CHARS"),
    };
  ...
  if (env.GEMINI_MCP_DAILY_TOKEN_LIMIT)
    merged.limits = {
      ...(merged.limits as object),
      maxTokensPerDay: parseIntEnv(env.GEMINI_MCP_DAILY_TOKEN_LIMIT, "GEMINI_MCP_DAILY_TOKEN_LIMIT"),
    };
  if (env.GEMINI_MCP_ENABLE_COST_ESTIMATES)
    merged.limits = {
      ...(merged.limits as object),
      enableCostEstimates: parseBooleanEnv(env.GEMINI_MCP_ENABLE_COST_ESTIMATES),
    };
  ...
  if (env.GEMINI_MCP_DEBUG)
    merged.logging = { ...(merged.logging as object), debug: parseBooleanEnv(env.GEMINI_MCP_DEBUG) };
  if (env.GEMINI_MCP_TRANSPORT)
    merged.transport = { ...(merged.transport as object), mode: env.GEMINI_MCP_TRANSPORT };
  if (env.GEMINI_MCP_HTTP_HOST)
    merged.transport = {
      ...(merged.transport as object),
      http: { ...((merged.transport as Record<string, unknown>)?.http as object), host: env.GEMINI_MCP_HTTP_HOST },
    };
  ...
  if (env.GEMINI_MCP_MAX_IMAGE_BYTES)
    merged.images = {
      ...
      maxBytes: parseIntEnv(env.GEMINI_MCP_MAX_IMAGE_BYTES, "GEMINI_MCP_MAX_IMAGE_BYTES"),
    };
  ...
  if (env.GEMINI_MCP_CONVERSATION_MAX_TURNS)
    merged.conversation = {
      ...
      maxTurns: parseIntEnv(env.GEMINI_MCP_CONVERSATION_MAX_TURNS, "GEMINI_MCP_CONVERSATION_MAX_TURNS"),
    };
  if (env.GEMINI_MCP_CONVERSATION_MAX_CHARS)
    merged.conversation = {
      ...(merged.conversation as object),
      maxTotalChars: parseIntEnv(env.GEMINI_MCP_CONVERSATION_MAX_CHARS, "GEMINI_MCP_CONVERSATION_MAX_CHARS"),
    };
